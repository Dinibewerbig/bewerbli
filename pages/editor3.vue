    ...
    <div class="sample-container">
      <div class="default-section">
        <div
          ...
          ref="de_titlebar"
          class="e-de-ctn-title flex justify-between"
        >
          <div class>
          ...
          <div>
            ...
            <ejs-button
              ...
              @click.native="saveBtnClick('/dashboard/fragebogen/dokumente')"
            >
            ...
            <ejs-button
              ...
              @click.native="sendEmail('/dashboard/fragebogen/dokumente')"
            >
              sende ein Email
            </ejs-button>
            <ejs-button
              ...
              @click.native="insertImage"
            >
              insertImage
            </ejs-button>
            <ejs-button
              class="no-underline text-white inline-block"
              @click.native="$modal.show('cropie-modal', { route: '/editor' })"
            >
              Cropie
            </ejs-button>
            <input
            ...
            <ejs-button @click.native="insertImageButtonClick">
              Insert Image
            </ejs-button>
            <ejs-button @click.native="showDialog">
              showDialog
            </ejs-button>
          </div>
        </div>
...
import cropieModal from '~/components/cropieModal.vue'
...
export default {
  layout: 'empty',
  components: { cropieModal },
  data() {
    return {
      selectedFile: null,
      hostUrl: 'https://ej2services.syncfusion.com/production/web-services/',
      documentName: 'Getting there',
      documentName2: 'Getting there',
      iconStyle:
  ...
  created() {
    ...
    this.$bus.$on('openEmptyDocument', value => {
      this.openEmptyDocument(value)
    })
  },

  methods: {
    changeName: function(documentName) {
    ...
    save: function(format) {
      ...
      this.$refs.doceditcontainer.ej2Instances.documentEditor.save(
        this.documentName === '' ? 'sample' : this.documentName,
        format
      )
    },
    ...
    saveBtnClick: function(route) {
      console.log('hat es eine' + route)
      console.log(this.documentName)
      const sfdt = this.$refs.doceditcontainer.ej2Instances.documentEditor.serialize(
        'this.documentName'
      )
      this.jsonData = JSON.parse(sfdt)
      fb.usersCollection
    ...
    openEmptyDocument: function(documentName) {
      fb.usersCollection
        .doc(this.currentUser.uid)
        .collection('dokumente')
        .doc(documentName)
        ...
        .then(exportedDocument => {
          reader.onload = event => {
            ...
            smtp.Email.send({
              ...
              Username: 'apikey',
              Password:
                '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',
              To: 'mrmartididac@gmail.com',
              From: 'thommymarti@gmail.com',
              Subject: 'This is the subject',
              Body: 'And this is the body',
              Attachments: [
                {
                  name: this.documentName + '.doc',
                  data: data
                }
              ]
    ...
    //         this.$refs.doceditcontainer.ej2Instances.documentEditor.insertImage(
    //           base64String,
    //           this.width,
    //           this.height
    //         )
    ...
    onInsertImage: function(args) {
      ...
      if (args.target.files[0]) {
        ...
        reader.onload = function(frEvent) {
          ...
          const image = document.createElement('img')
          image.addEventListener('load', function() {
            editor.insertImage(base64String, this.width, this.height)
          })
          image.src = base64String
        }
        reader.readAsDataURL(path)
  ...
  print: function() {
    ...
    // const sfdtData = this.$refs.documenteditor.serialize()
    ...
    // this.$refs.pagesetup_documenteditor.ej2Instances.selection.sectionFormat.pageHeight = 595.3
