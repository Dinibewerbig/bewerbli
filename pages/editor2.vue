  ...
  <div class="control-section">
    <div class="sample-container">
      <div class="default-section">
        <div id="documenteditor_titlebar" ref="de_titlebar" class="e-de-ctn-title  ">
          <ejs-dropdownbutton
            ref="deexport"
            :style="dropDownStyle"
            ...
            :items="templateitems"
            content="Choose"
            :select="onOpenTemplate"
            title="Open the saved document." 
          />
          
          <div id="documenteditor_title_contentEditor" class="single-line" style="border: 1px solid #2B3481;position:relative;left:550px;" title="Document Name. Click or tap to rename this document." contenteditable="false">
            <label id="documenteditor_title_name" :style="titileStyle">
              {{ documentName }}
            </label>
          </div>    
          <input
            ...
            @change="fileChange"
          >   
          <ejs-button id="de-print" :style="iconStyle" :icon-css="printIconCss" title="Print this document (Ctrl+P)." @click.native="printBtnClick">
            Print
          </ejs-button>
          <ejs-button id="de-print" :style="iconStyle" :icon-css="exportIconCss" title="Save this document." @click.native="saveBtnClick">
            Save
          </ejs-button>              
        </div>`
...
export default {
  ...
  data() {
    return {
      ...
      dropDownStyle:
        'background: transparent;box-shadow:none;border-color: transparent;border-radius: 2px;color:#ffffff;font-size:12px;text-transform:capitalize;margin-top:-32px;height:28px;font-weight:400;font-family:inherit;',
      titileStyle:
  ...
  mounted() {
    this.$nextTick(function() {
      this.titleBarHeight = document.getElementById(
        'documenteditor_titlebar'
      ).offsetHeight
      this.deexport = this.$refs.deexport.ej2Instances
    })
    document
      .getElementById('documenteditor_title_contentEditor')
      .addEventListener('keydown', function(e) {
        this.titleBarKeydownEvent(e)
      })
    document
      .getElementById('documenteditor_title_contentEditor')
      .addEventListener('blur', function() {
        this.titleBarBlurEvent()
      })
    document
      .getElementById('documenteditor_title_contentEditor')
      .addEventListener('click', function() {
        this.titleBarClickEvent()
      })
  },
  // created: function() {
  ...
  // },
  methods: {
    onOpenTemplate: function(args) {
      if (this.documentContentChange) {
        const msg = confirm(
      ...
      const data = this.$refs.jsonData[args.item.text]
      const waitingPopUp = document.getElementById('waiting-popup')
      const inActiveDiv = document.getElementById('popup-overlay')
      waitingPopUp.style.display = 'block'
      inActiveDiv.style.display = 'block'
      this.$refs.documenteditor.ej2Instances.documentName = args.item.text
      this.$refs.documenteditor.ej2Instances.open(JSON.stringify(data))
    },
    ...
    save: function(format) {
      ...
      this.$refs.documenteditor.ej2Instances.save(
        this.$refs.documenteditor.ej2Instances.documentName === ''
          ? 'sample'
          : this.$refs.documenteditor.ej2Instances.documentName,
        format
    ...
    saveBtnClick: function() {
      const sfdt = this.$refs.documenteditor.ej2Instances.serialize()
      const fileName =
        this.$refs.documenteditor.ej2Instances.documentName === ''
          ? 'Untitled Document'
          : this.$refs.documenteditor.ej2Instances.documentName
      this.$refs.jsonData[fileName] = JSON.parse(sfdt)
      this.updateTemplateDropDown()
    },
    updateTemplateDropDown: function() {
      this.deexport.items = []
      const value = Object.keys(this.$refs.jsonData)
      const sample = []
      for (let i = 0; i < value.length; i++) {
        sample.push({ text: value[i] })
      }
      this.deexport.items = sample
    },
    ...
    titleBarBlurEvent: function(args) {
      ...
      this.$refs.documenteditor.ej2Instances.documentName = document.getElementById(
    ...
    fileChange: function(args) {
      if (args.target.files[0]) {
        const path = args.target.files[0]
        if (path.name.substr(path.name.lastIndexOf('.')) === '.sfdt') {
          ...
          fileReader.onload = e => {
            ...
            this.$refs.documenteditor.ej2Instances.open(
              contents
            ).this.$refs.jsonData[
              this.$refs.documenteditor.ej2Instances.documentName
            ] = JSON.parse(contents)
    ...
    loadDefaultDocument: function() {
      ...
      // eslint-disable-next-line standard/computed-property-even-spacing
      waitingPopUp.style.display = 'none'.this.$refs.jsonData[
        this.$refs.documenteditor.ej2Instances.documentName
      ] = JSON.parse(defaultDocument)
      this.updateTemplateDropDown()
    },
              ...
              blocks: [
                {
                  ...
                  characterFormat: {},
                  inlines: [{ characterFormat: {}, text: 'Template 1' }]
                }
              ],
              headersFooters: {
                header: {
              ...
              blocks: [
                {
                  paragraphFormat: { styleName: 'Normal', listFormat: {} },
                  characterFormat: {},
                  inlines: [{ characterFormat: {}, text: 'Template 2' }]
                }
              ],
              headersFooters: {
                header: {
              ...
              blocks: [
                {
                  paragraphFormat: { styleName: 'Normal', listFormat: {} },
                  characterFormat: {},
                  inlines: [{ characterFormat: {}, text: 'Template 3' }]
                }
              ],
              headersFooters: {
                header: {
