<template>
  <section class="container">
    ...
    <div class="bs-callout-info bs-callout">
      ...
      <p>
        First, <a href="https://www.google.com/accounts" target="_blank">
          sign up
        </a> for a
    ...
    <div class="jumbotron well text-center">
      <!-- Loading -->
      <div v-show="loading" class="text-danger lead">
        <p>{{ loading }}...</p>
      </div>
      <!-- Errors -->
      <div v-show="response" class="text-danger lead">
        <p>{{ response }}</p>
      </div>
      ...
      <div class="auth-btns">
        <div v-if="isAuth">
          ...
          <br>
          <a class="button--grey" @click="signOut">
            <span
          ...
          <a class="button--grey" @click="disconnect">
            <span
              class="glyphicon glyphicon-remove"
              aria-hidden="true"
            /> Revoke access
          </a>
          <a class="button--grey" @click="addScope">
            <span
        ...
        <div v-else>
          ...
          <br>
          <a class="button--green" @click="signIn()">
            <span
...
export default {
  ...
  methods: {
    ...
    updateAuthStatus() {
      // var googleUser = this.auth2.getAuthInstance().currentUser.get()
      console.log('inside updateAuthStatus')
      console.log(this.authGoogle.currentUser)

      const credential = firebase.auth.GoogleAuthProvider.credential(
        this.authGoogle.currentUser.getAuthResponse().id_token
      )
      ...
      firebase
        ...
        .then(credential => {
          ...
          console.log('CurrentUser in sign in...' + credential.user)
          fb.usersCollection
        ...
        window.setTimeout(function() {
          self.authGoogle
            .addScope(
              ...
              self.onAddScopeSuccess,
              self.onAddScopeError
            )
            .then(function() {})
        }, 1000)
      } else {
    ...
    toggleLoading: function() {
      this.loading =
        this.loading === '' ? (this.isAuth ? 'Logout' : 'Login') : ''
    },
